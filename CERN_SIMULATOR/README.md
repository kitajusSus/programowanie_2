...
graph TD
    subgraph "Legenda"
        ...
        L2 --> L3[("Struktura Danych")]; %% Kształt bazy danych/cylindra
        L3 --> L4(("(Obiekt Graficzny)")); %% Kształt koła
        L4 --> L5{"Decyzja / Pętla"}; %% Kształt rombu
    end
    ...
    subgraph "Interakcja Użytkownika - Kontrolki"
        P1Edits{"Pola Edycji Cz. 1"} -- "zmiana wartości" --> UP_CB(["Callback: @updateParticles"]); %% Kształt stadionu
    ...
    subgraph "Sterowanie Symulacją"
        ...
        StopSim -- "ustawia" --> SD_Run_False[("simData.isRunning = false")]; %% Użyto innego ID dla jasności
        ResetSim -- "wywołuje" --> StopSim;
        ResetSim -- "wywołuje" --> UP;
        ResetSim -- "resetuje" --> SD_Time; %% Odnosi się do istniejącego węzła SD_Time
    ...
    subgraph "Pętla Symulacji"
        ...
        %% Poprawiona składnia dla akcji sprawdzania/modyfikacji jako etykiet krawędzi
        USim -- "sprawdza/modyfikuje (Odbicia od ścian)" --> SD_P1P2_VelPos;
        USim -- "sprawdza/modyfikuje (Kolizje)" --> SD_P1P2_VelPos;
    ...
    subgraph "Obliczenia i Rysowanie Pomocnicze"
        CC -- "odczytuje" --> SD_P1P2;
        CC -- "zwraca" --> Sila["Siła"]; %% Węzeł reprezentujący wynik
